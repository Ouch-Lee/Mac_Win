...
uint32_t lcd_read_point(uint16_t x, uint16_t y)
{
    ...
    g <<= 8;
    return (((r >> 11) << 11) | ((g >> 10) << 5) | (b >> 11));  /* ILI9341/NT35310/NT35510/ST7789需要公式转换一下 */
}
...
void lcd_init(void)
{
    ...
    /* FSMC读时序控制寄存器 */
    fsmc_read_handle.AddressSetupTime = 0x0F;           /* 地址建立时间(ADDSET)为15个fmc_ker_ck 1/168=6*16=96ns */
    fsmc_read_handle.AddressHoldTime = 0x00;
    fsmc_read_handle.DataSetupTime = 60;                /* 数据保存时间(DATAST)为60个fmc_ker_ck=6*60=360ns */
                                                        /* 因为液晶驱动IC的读数据的时候,速度不能太快,尤其是个别奇葩芯片 */
    fsmc_read_handle.AccessMode = FSMC_ACCESS_MODE_A;   /* 模式A */
    /* FSMC写时序控制寄存器 */
    fsmc_write_handle.BusTurnAroundDuration = 0;
    fsmc_write_handle.AddressSetupTime = 9;             /* 地址建立时间(ADDSET)为9个fmc_ker_ck=6*9=54ns */
    fsmc_write_handle.AddressHoldTime = 0x00;
    fsmc_write_handle.DataSetupTime = 9;                /* 数据保存时间(DATAST)为9个fmc_ker_ck=6*8=54ns */
                                                        /* 9个fmc_ker_ck（fmc_ker_ck=168Mhz）,某些液晶驱动IC的写信号脉宽，最少也得50ns */
