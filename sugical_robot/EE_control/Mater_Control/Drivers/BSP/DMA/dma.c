 ...
 * @retval      无
 */
void dma_map_config(DMA_Stream_TypeDef* DMAx_CHx,uint32_t src,uint32_t dest)
{
    if ((uint32_t)DMAx_CHx > (uint32_t)DMA2)     /* 大于DMA1_Channel7, 则为DMA2的通道了 */
    {
 ...
 *                            某个外设对应哪个DMA, 哪个通道, 请参考<<STM32F4xx中文参考手册.pdf>> 9.3.3节
 *                            必须设置正确的DMA及通道, 才能正常使用! 
 * @param       ndtr       : 数据传输量
 * @retval      无
 */
void dma_enable(DMA_Stream_TypeDef *dmax_chy, uint16_t ndtr)
{
    __HAL_DMA_DISABLE(&g_dma_handle);           /* 关闭DMA传输 */
    g_dma_handle.Instance->NDTR = ndtr;       /* 重设DMA传输数据量 */
...
{
    dma_map_config(DMA2_Stream7, mar, (uint32_t)&USART1->DR);   /* USART1_TX使用的DMA通道为: DMA1_Channel4 */
}
