...
设想的从端手术机器人配置如下（图2）：柔性机械臂搭载在双通道的内窥镜上；其近端连续体由同向的单元连接而成，在一对cable的驱动下可以朝两侧弯曲，以增大远端连续体之间的间距，提供较好的三角测量性；远端连续体相邻的单元正交排布，因此具有可以实现两个方向偏转，末端器械通过微型轴承连接在远端连续体上，具有绕轴旋转和开合的自由度。
...
生理抖动产生的一个原因是手部悬空[]。通过观察操纵者手指末端的抖动幅度在手臂被托举状态下远小于悬空状态，因此商业微创手术机器人的操纵台基本都设计了小臂托衬结构，该设计的另外一个优点是缓解手术时手臂维持长时间悬臂的疲劳。我们通过采集两种状态下电机的角度信息，计算出操纵台末端位置信息，并将其可视化在图中。
...
尽管在设计操纵台时我们力求主从两端拥有相同的运动自由度，但是由于刚性连杆和柔性连续体结构上的区别，主从两端的**构型空间**不相同，该部分主要通过对主从两端进行运动学建模，并推导从主端驱动空间到从端构型空间的映射策略。

图（）显示了主从系统的integrated kinematic drawing。对于主端刚性连杆结构，我们沿用DH标准为每个关节配置了坐标系，并构建了对应的DH table；对于从端柔性连续体，常规DH法存在什么问题[ ]，因此对于从端模型我们采用的常曲率模型对其建模；

为了符合直觉控制的规定，从端执行器的位姿构型应该与操纵者手部位姿保持一致，因此需要满足以下两约束：

l 约束一：在标定模式（不进行运动缩放）从端柔性手术机械臂末端朝向要时刻主端手指的朝向保持一致

l 约束二：从端沿内窥镜通道轴向方向的伸缩运动与主端小臂前后的运动呈比例缩放关系

我们将  由于远近两段连续体的控制是独立的，通过将两者进行解耦可以大大简化求解难度；

——————删掉————————

若需要满足约束一中关系，远近两段对应的齐次变换矩阵中的选择矩阵部分应该相等，对于近端这个问题较为简单，

单独分析主端小臂关节的旋转，对应的旋转矩阵为（公式）
$$
公式
$$
根据公式（4.4），近端单自由度连续体齐次变化矩阵中的旋转矩阵                               
$$
公式
$$
对于远端，尝试类比上述过程，但是发现
