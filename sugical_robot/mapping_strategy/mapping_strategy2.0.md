...
d_2 = \sqrt{x_c^2 + y_c^2}
$$
上图中红色箭头为电机的零位，因为电机固定在连杆BC上，所有红色箭头随BC运动而改变，绿色虚线描述坐标系1与坐标系3之间的关系，也即坐标系3的零位，如图所示，五连杆运动时两者之间会出现一个偏差。换而言之，当电机不动，也即$\beta_3$ 保持0时，五连杆的运动会使得红色箭头在绿色虚线坐标系下角度改变，需要人为的减去这个$\Delta \theta$ ；

通过几何关系可以求解：

当电机固定在连杆BC上时（左手）
$$
\theta_3 = \beta - \Delta\theta \\
where,  \Delta\theta =  \theta_1 -  u_2 + 0.2\pi \\
$$
当电机固定在连杆CD上时（右手）
$$
\theta_3 = \beta - \Delta\theta \\
where,  \Delta\theta =  \theta_1 + u_4 - 0.2\pi \\
$$
