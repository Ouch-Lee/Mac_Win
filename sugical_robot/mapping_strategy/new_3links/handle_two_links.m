% this script is used to calculate the DH matrix
...
% % d2 = sym(60.1);
% % theta2 = 0
% A2 = simplify([cos(theta2 + pi/2), -sin(theta2 + pi/2)*cos(alp2), sin(theta2 + pi/2)*sin(alp2), a2*cos(theta2 + pi/2)
%     sin(theta2 + pi/2), cos(theta2 + pi/2)*cos(alp2), -cos(theta2 + pi/2)*sin(alp2), a2*sin(theta2 + pi/2)
...
% T = simplify(A1*A2*At2* A3 * At3 * A4 * A5);
